...
class SnapClassifier {
     ...
     */
    fun inference(): List<Float> {
        tensor.load(recorder)
        Log.d(TAG, tensor.tensorBuffer.shape.joinToString(","))
        val output = classifier.classify(tensor)
        Log.d(TAG, output.toString())
        val r: List<Float> = listOf(output[0].categories.find { it.label == "Bark" }!!.score, output[0].categories.find { it.label == "Meow" }!!.score)
        return r
    }
     ...
     */
    interface DetectorListener {
        fun onResults(score: List<Float>)
    }
